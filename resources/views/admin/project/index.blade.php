							...
							<tbody>
								@foreach($data as $key => $val)
									<tr class="parent">
										...
										<td class="treeview">{{ $val -> cinema_id }}</td>
										<td>{{ $val -> hall_id }}</td>
										<td>{{ $val -> movie_id }}</td>
										<td>{{ $val -> date_id }}</td>
										<td style="font-weight:bold;color:black;">{{ $val -> time_id }}</td>	
										<td><a href="{{ url('/admin/project/edit') }}/{{ $val ->id }}">编辑</a> <a href="{{ url('/admin/project/delete') }}/{{ $val ->id }}">删除</a></td>
									</tr>
								@endforeach
							</tbody>
						</table>
