							...
							<tbody>
								@foreach($data as $key => $val)
									<tr class="parent">
										...
										<td class="treeview">{{ $val -> hall_name }}</td>
										@foreach($res as $k => $v)
											@if($val -> tid == $v -> id)
											<td style="font-weight:bold;color:black;">{{ $v -> cinema_name }}</td>
											@endif
										@endforeach
										<td><a href="{{ url('/admin/hall/edit') }}/{{ $val ->id }}">编辑</a> <a href="{{ url('/admin/hall/delete') }}/{{ $val ->id }}">删除</a></td>
									</tr>
								@endforeach
							</tbody>
						</table>
